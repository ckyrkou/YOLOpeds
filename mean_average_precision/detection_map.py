...
class DetectionMAP:
    ...
    def evaluate_(IoUmask, accumulators, pred_classes, pred_conf, gt_classes, confidence_threshold):
        ...
        for i, acc in enumerate(accumulators):
            ...
            if (gt_number == 0 and pred_number == 0):
                continue
            if pred_number == 0:
    ...
    def compute_mAP(self, interpolated=True):
        ...
        mAP = sum(mean_average_precision)/len(mean_average_precision)

        return mAP,average_precision,precisions,recalls
    ...
    def plot(self, interpolated=True, class_names=None):
        ...
        :return:
        """
        grid = int(math.ceil(math.sqrt(max(self.n_class,4))))
        if grid == 1:
            fig, ax = plt.subplots()
            mean_average_precision = []
